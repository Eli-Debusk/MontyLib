...
import frc.montylib.constants.VariableSpeedMode;

//TODO: move accel limiting to outside the divisor of mechanical speed
...
public class ConfigurableMotionProfile {
     ...
     */
    public double calculate(int mode, double value, double variable_speed_factor) {
        return variableSpeedEnabled ? 
            accelerationLimitingEnabled ? 
                accelLimiter.calculate(value * (dualSpeedController.calculate(mode, variable_speed_factor)))
                :
                value * (dualSpeedController.calculate(mode, variable_speed_factor))
        : 
        0.0;
    }
     ...
     * @return a calculated motor output to achieve a desired velocity
     */
    public double calculate(int mode, double value, double primary_variable_speed_factor, double secondary_variable_speed_factor) {
        return variableSpeedEnabled ? 
            accelerationLimitingEnabled ? 
                accelLimiter.calculate(value * (triSpeedController.calculate(mode, primary_variable_speed_factor, secondary_variable_speed_factor)))
                :
                value * (triSpeedController.calculate(mode, primary_variable_speed_factor, secondary_variable_speed_factor))
        : 
        0.0;
    }
}
...
class dualSpeedMotionController {
    ...
    public double calculate(int mode, double factor) {
        switch (mode) {
            case VariableSpeedMode.INCREASE: 
                return primarySpeed + (MontyMath.valueDifference(primarySpeed, secondarySpeed) * factor);
            case VariableSpeedMode.DECREASE:  
                return primarySpeed - (MontyMath.valueDifference(primarySpeed, secondarySpeed) * factor);
...
class triSpeedMotionController {
    ...
    public double calculate(int mode, double factor1, double factor2) {
        switch(mode) {
            case VariableSpeedMode.INCREASE: 
                return primarySpeed 
